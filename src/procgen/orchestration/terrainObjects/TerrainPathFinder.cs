...
public class TerrainPathFinder : IPoolable {
	...
	void DebugDrawClosedSet() {
		foreach (DPoint p in closedset) {
			...
			if (came_from.TryGetValue(p, out o)) {
				DebugDrawer.DrawLine(((Vector2)o).xoy(), ((Vector2)p).xoy(), Colors.Yellow, 1000);
				DebugDrawer.DrawCircle(((Vector2)p), 1f, 4, Colors.Red, 1000);
			}
		}
	}
	...
	float GetWaterCost(DPoint a, DPoint b, bool lowerBound) {
		...
		float sidewaysPenalty = (sm - sm.Dot( sg) / sg.Dot(sg) * sg).LengthSquared() * 0.001f;
